...
function create_etd_mdf($f_in){
	...
	$amp_marker = "<--This-is-an-ampersand-->";
	//$simple = str_replace("&",$amp_marker,$simple);
	$p = xml_parser_create();
...
//Make sure you put the right escape characters in
	...
	$final_string = "<?xml version=\"1.0\" encoding=\"ISO-8859-1\"?>\n<dublin_core>\n";
	
	foreach ($vals as $top){

		if(!empty($top['value'])){
		//$top['value'] = str_replace($amp_marker,"&",$top['value']);
		$content = htmlspecialchars_decode($top['value']);
