...
if ( typeof global.startBatch == "undefined" ) {
	...
	cName: "mySummary.xls",
	cValue: "dc.contributor.author\tdc.title\tdc.date\tdc.date.issued\tdc.publisher\tdc.rights\tdc.description.abstract\tdc.subject.author\tdc.contributor.advisor\tdc.contributor.secondreader\tetd.thesisdegree.name\tetd.thesisdegree.level\tetd.thesisdegree.discipline\tdc.description.service\tdc.contributor.department\tpath\r\n"});
}
/*
else
{
	...
	try
	{
		if(re.test(dataLine))
			...
			if(trim(captured[1]))
			{
				...
				captured[1] = toTitleCaseFirstChar(captured[1]);
			...
			else
			{
				if(trim(captured[0]))
				{
					...
					captured[0] = toTitleCaseFirstChar(captured[0]);
					OLine += trim(captured[0]);
				}
				else
				{
...
/******************************************* Begin: Rights ********************************************/
//This is going to be a blanket statement...
	OLine += "Approved for public release; distribution is unlimited";
	OLine += "\t";
/******************************************* End: Rights ********************************************/
...
/****************************************** Begin: Department **********************************************/
	...
	var Department = "";
	re = /(?:for)(?:\s{0,4})(?:the)(?:\s{0,4})(?:degree)(?:\s{0,4})(?:of)(?:\s{0,4})(.*)(?:from)(?:\s{0,4})(?:the)/i;
	re = /(?:department)(?:\s{0,4})(?:of)(?:\s{0,4})(.*)(?:\n)/i;
	try{
		if(re.test(apDataLine))
		{
			captured = re.exec(apDataLine);
			len = captured.length;
			if(trim(captured[1]))
			{
				captured[1] = trim(captured[1]);
				if(captured[1].substring(captured[1].length-2).toLowerCase() == "iv")
					captured[1] = captured[1].substring(0,captured[1].length-2);
				Department = trim(captured[1]);
			...
			else
			{
				if(trim(captured[0]))
				{
					console.println("Department: There was a problem with the match seperation, so the ENTIRE match was used...");
					Department += trim(captured[0]);
					DeptFlag = true;
				}
				else
				{
					//OLine += "ERROR";
					console.println("Unable to capture Department...");
				}
			}
			captured = [];
			re = "";
		...
		{
			//OLine += "ERROR";
			console.println("Fail in matching Department...");
		}
	}
	catch(e)
	{
