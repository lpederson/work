...
/*******************************************Begin: Finding Title*********************************************/
	...
	re = /(?:4\.?\s{0,4})(?:TITLE)(?:\s{0,4})(?:AND?)(?:\s{0,4})(?:SUBTITLE?)(?:[:]{0,3})(?:\s{0,4})(.*)(?:5[a-f]?)(?:\.?\s{0,4})/i;
	try
	{
		if(re.test(dataLine))
			{
			...
			if(trim(captured[1]))
			{
				...
				captured[1] = captured[1].replace("5. FUNDING","");
	...
	try
	{
		if(re.test(dataLine))
		{
			...
			if(trim(captured[1]))
			{
				...
				captured[1] = captured[1].replace(/(\()(\s{0,4})(maximum)(\s{0,4})(\d{3})(\s{0,4})(words)(\s{0,4})(\.\))(\s{0,4})/i,"");
				captured[1] = captured[1].replace("(Maximum 200 words.)","");
